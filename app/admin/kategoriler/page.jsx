...
export default function AdminCategoriesPage() {
  ...
  const fetchCategories = async () => {
    try {
      ...
      if (allRes.ok) {
        ...
        const filteredCategories = categoriesData.filter((c) => c && c._id);
      ...
      if (parentsRes.ok) {
        ...
        // Null/undefined kategorileri filtrele
        const filteredParents = parentsCategories.filter((c) => c && c._id);
        setParentCategories(filteredParents);
      } else {
        // Fallback: Tüm kategorilerden parent olmayanları al
        const mainCats = categories.filter((c) => !c.parent || (typeof c.parent === 'object' && !c.parent._id));
      ...
      if (mainCategoryId) {
        if (editingCategory) {
          ...
          for (const subCat of validSubCategories) {
            const existingSub = existingSubs.find((s) => s.slug === subCat.slug);
            if (existingSub) {
  ...
  const updateSubCategoryFields = (index, updates) => {
    const updated = [...subCategories];
    ...
    if (!parentId) {
      try {
        ...
        if (subRes.ok) {
          ...
          const subs = (subData.categories || []).map((sub) => ({
            name: sub.name,
            slug: sub.slug,
                          ...
                          <div className="mb-4 pb-4 border-b border-gray-200">
                            ...
                            <div className="space-y-2">
                              {category.subCategories
                                .filter((subCat) => subCat && subCat._id) // null kontrolü
                                .map((subCat) => (
