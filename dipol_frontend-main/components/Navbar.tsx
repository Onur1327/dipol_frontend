...
export default function Navbar() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (process.env.NODE_ENV === 'development') {
            ...
            console.log('Aktif kategoriler:', activeCategories.length);
            console.log('Ana kategoriler:', activeMainCategories.length);
            console.log('Ana kategori isimleri:', activeMainCategories.map((c: any) => c.name));
          }
        } else {
  ...
  // Ana kategorileri sırala (order'a göre)
  const sortedMainCategories = [...mainCategories].sort((a: any, b: any) => (a.order || 0) - (b.order || 0));

  // Sadece ana kategorileri navbar'da göster, alt kategoriler dropdown'da
  ...
  const categoryNavItems = sortedMainCategories.map((cat: any) => {
    const categoryId = cat._id ? cat._id.toString() : '';
    const subcategories = getSubcategories(categoryId);
    return {
      name: cat.name,
      href: `/urunler?category=${cat.slug || cat._id}`,
      hasDropdown: subcategories.length > 0, // Alt kategori varsa dropdown göster
  ...
  return (
    <>
      {/* Top Promotional Bar */}
      <div className="bg-black text-white text-sm py-2 fixed w-full top-0 z-50 overflow-hidden">
