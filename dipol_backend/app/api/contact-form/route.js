...
export async function POST(request) {
  try {
    ...
    if (!emailRegex.test(email)) {
      return NextResponse.json({ error: 'Geçerli bir e-posta adresi giriniz' }, { status: 400, headers: getCorsHeaders(request) });
    }
    ...
    const emailHtml = `
      ...
      <body>
        <div class="container">
          <div class="header"><h1>Yeni İletişim Formu Mesajı</h1></div>
          <div class="content">
            <p>Web sitenizden yeni bir iletişim formu mesajı alındı:</p>
            <div class="info-box">
              <div class="label">Gönderen Bilgileri:</div>
              <div class="value"><strong>Ad Soyad:</strong> ${name}<br><strong>E-posta:</strong> ${email}</div>
            </div>
            <div class="info-box"><div class="label">Konu:</div><div class="value">${subject}</div></div>
            <div class="message-box"><div class="label">Mesaj:</div><div class="value">${message.replace(/\n/g, '<br>')}</div></div>
    ...
    try {
      ...
      return NextResponse.json({ error: 'Mesaj gönderilemedi. Lütfen tekrar deneyin.' }, { status: 500, headers: getCorsHeaders(request) });
    }
    ...
    return NextResponse.json({ error: error.message || 'Mesaj gönderilemedi' }, { status: 500, headers: getCorsHeaders(request) });
  }
}
