...
export async function sendPasswordResetEmail(email: string, resetToken: string): Promise<void> {
  // Production'da Vercel URL'ini kullan, development'ta localhost
  const frontendUrl = process.env.FRONTEND_URL || process.env.NEXT_PUBLIC_FRONTEND_URL || 
    (process.env.NODE_ENV === 'production' ? 'https://dipol-fe.vercel.app' : 'http://localhost:3001');
...
export async function sendEmailVerificationEmail(email: string, verificationToken: string): Promise<void> {
  const verificationUrl = `${process.env.FRONTEND_URL || process.env.NEXT_PUBLIC_FRONTEND_URL || 'https://dipol-fe.vercel.app'}/email-onay?token=${verificationToken}`;
