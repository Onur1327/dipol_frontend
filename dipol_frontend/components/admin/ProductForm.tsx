...
export default function ProductForm({ product }: ProductFormProps) {
  ...
  const parseColorImages = (colorImagesData: any): { [color: string]: string[] } => {
  ...
  const [newColorImageUrl, setNewColorImageUrl] = useState<{ [color: string]: string }>({});
  const [tempColorImageUrl, setTempColorImageUrl] = useState<{ [color: string]: string }>({});
  
  // Renk/Beden kombinasyonuna göre stok yönetimi
  const parseColorSizeStock = (colorSizeStockData: any): { [color: string]: { [size: string]: number } } => {
    ...
    if (colorSizeStockData instanceof Map || colorSizeStockData.constructor?.name === 'Map') {
      const result: { [color: string]: { [size: string]: number } } = {};
      colorSizeStockData.forEach((sizeMap: Map<string, number>, color: string) => {
    ...
    if (typeof colorSizeStockData === 'object') {
      const result: { [color: string]: { [size: string]: number } } = {};
      Object.entries(colorSizeStockData).forEach(([color, sizeMap]: [string, any]) => {
        if (sizeMap instanceof Map) {
          result[color] = Object.fromEntries(sizeMap);
        } else {
          result[color] = sizeMap;
  ...
  const [colorSizeStock, setColorSizeStock] = useState<{ [color: string]: { [size: string]: number } }>(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const submitData = {
        ...
        comparePrice: formData.comparePrice ? parseFloat(formData.comparePrice as any) : undefined,
      };
      ...
      <div className="space-y-4">
        ...
        {formData.colors.map((color: string) => (
          <div key={color} className="border border-gray-200 rounded-lg p-4">
            ...
            <div className="grid grid-cols-4 gap-4">
              {colorImages[color]?.map((image: string, index: number) => {
                const isBase64 = image.startsWith('data:image');
                return (
                  <div key={index} className="relative group">
                    <div className="relative aspect-square rounded-lg overflow-hidden border-2 border-gray-200">
